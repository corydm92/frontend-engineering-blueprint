...
This ADR establishes the Layer 0 foundation — documenting framework, language, runtime, and tooling versions that all contributors must follow.
...
Motivations
- Align new projects with the organization’s Frontend Engineering Blueprint.
- Prevent environment drift through explicit version control.
- Simplify onboarding and reduce ambiguity in setup steps.
- Serve as a single source of truth for the project’s technical foundation.
...
Decision
Define the Layer 0 foundation — all frameworks, runtimes, and tools chosen for this project.
...
Documentation
- /docs directory aligned with the Frontend Engineering Blueprint (Layers 0–6).
...
Option 2: <description> — rejected because <reason>.
Option 3: <description> — rejected because <reason>.

If none, note “No viable alternatives at time of decision.”
...
It replaces the v15 foundation with modern caching, bundling, and runtime primitives — ensuring long-term maintainability and MAANG-level performance parity.
...
- Rust-based incremental bundler providing 2–5× faster build and refresh times.
...
- Unified global and async state model aligned with Next 16’s opt-in caching.
...
Documentation
- /docs directory aligned with Blueprint Layers 0–6.
...
Alternatives Considered
- Next.js 15 — rejected for deprecated caching defaults and partial Turbopack support.
- Node 20 LTS — rejected due to missing Web Streams and diagnostic channel improvements.
- pnpm 9.x — rejected for slower lifecycle scripts and weaker workspace handling.
- Vitest — rejected to maintain Jest stability and alignment with existing test infrastructure.
