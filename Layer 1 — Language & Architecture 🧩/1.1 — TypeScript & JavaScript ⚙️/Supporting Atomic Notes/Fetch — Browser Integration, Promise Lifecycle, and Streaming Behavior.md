...
but all actual I/O and state changes occur inside the browser’s internal subsystems, not the JS engine.
...
The browser’s network operations occur independently from JS.
...
One-liner summary:
fetch() is a coordination layer between JavaScript’s microtask system and the browser’s network engine — resolving early for headers, streaming late for data, and surfacing both through Promises that integrate seamlessly with the event loop.