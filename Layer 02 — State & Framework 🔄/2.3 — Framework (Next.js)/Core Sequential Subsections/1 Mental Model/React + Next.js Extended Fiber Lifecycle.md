...
- Next.js owns routing, data fetching, and determines when to call React’s render engine.
...
calling ReactDOM.hydrateRoot(document.getElementById("\_\_next"), <App />).
...
2. The DOM referenced here is the real browser DOM built from server HTML.
   React doesn’t re-render it — it attaches behavior and ownership.
...
Steps 1–3: Server work
Steps 4–6: Client work

## 1 User clicks a <Link /> in the browser.

- The Next.js router intercepts the click (no full reload).

## 2 The router fetches the new route’s RSC stream from the Next.js server.
