...
- Promise B represents the _future result_ of res.json(), not the fetch response itself.
...
- Note: each .then() in this chain is linked to the _previous_ promise, not the same one.
  This means when Promise A fulfills, only the first .then() callback (res => res.json()) is queued.
  When Promise B later fulfills, its own .then() (data => console.log(...)) is queued in a _subsequent microtask cycle_.
