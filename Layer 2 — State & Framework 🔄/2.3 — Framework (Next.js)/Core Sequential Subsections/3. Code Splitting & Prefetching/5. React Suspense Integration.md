...
Next.js integrates ReactÍs Suspense boundaries for async rendering on both the server and client, enabling partial rendering and streaming.
...
Result:
The fallback (loading.tsx) renders instantly while the pageÍs async data or components stream in when ready.
