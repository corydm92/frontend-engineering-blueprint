...
When a component’s output depends only on its props, state, and context—and never mutates them—it becomes predictable and efficient.
...
user.name = user.name.toUpperCase(); // ❌ Mutation during render
...
4. React’s render output now diverges from expected state.

Fix:
function Greeting({ user }) {
const upperName = user.name.toUpperCase(); // ✅ No mutation
...
React’s render phase is pure, but event handlers are safe for mutations—they only run after React commits updates.
...
setCount(count + 1); // ✅ Safe, runs after commit
}
...
function addTodo() {
todos.push({ id: 2, text: "Stay pure" }); // ❌ Mutates state directly
...
function addTodo() {
setTodos([...todos, { id: 2, text: "Stay pure" }]); // ✅ Creates new reference
}
